...
def _rating_band(value: float, low: float, watch: float, alert: float, danger: float) -> int:
    """Map a share to a 1-10 risk score using document-specific four-band thresholds."""
    if value >= danger:
        return 9
    if value >= alert:
        return 7
...
def region_score(metrics: ConcentrationMetrics) -> int:
    ...
    province = _rating_band(metrics.single_province_share, 0.15, 0.25, 0.35, 0.50)
    weak = _rating_band(metrics.weak_region_share, 0.05, 0.10, 0.20, 0.35)
...
def rating_score(metrics: ConcentrationMetrics) -> int:
    ...
    aaa = _rating_band(metrics.aaa_share, 0.30, 0.50, 0.70, 0.85)
    pseudo = _rating_band(metrics.pseudo_high_rating_share, 0.05, 0.15, 0.30, 0.30)
...
def maturity_score(metrics: ConcentrationMetrics) -> int:
    ...
    m12 = _rating_band(metrics.maturity_12m_share, 0.30, 0.50, 0.70, 0.70)
    peak = _rating_band(metrics.single_month_peak, 0.10, 0.20, 0.30, 0.30)
    return max(m12, peak)


def channel_score(metrics: ConcentrationMetrics) -> int:
    """D5: Financing-channel concentration (top channel share + contraction flag)."""
    base = _rating_band(metrics.top_channel_share, 0.30, 0.50, 0.70, 0.90)
    if metrics.top_channel_is_contracting and base < 9:
        base += 2
    return _clamp(base, 2, 10)
