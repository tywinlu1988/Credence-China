...
- `version/<version>/` —— 该版本的只读归档快照。**GitHub 清理后：主仓库 git 仅跟踪当前一个 release 快照（现为 `version/v0.8.6-release/`）；历史快照已从 git 跟踪中移除，仍可从维护者本地磁盘与 git 提交历史找回（全部发布提交均在 master 提交图内，按 SHA 检出即可）。** 一旦快照创建，仅允许通过补丁分支修正明显错误，不允许继续功能迭代。
