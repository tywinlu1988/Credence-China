...
**13 行业 · 16 工作路径 · 10 编码引擎 · 29 引擎文档 · 16 报告模板**

### 能力亮点

- **四段链技能**：路由 → 分析 → 报告 → 质检，意图确认后才执行，不脑补。
- **8 条可复算编码引擎路径**（WP-M0-01 旗舰聚合 / WP-M0-02 LGD+外部支持 / WP-M4-01 集中度 / WP-M4-02 传染矩阵 / WP-M4-03 SRI / WP-M4-04 组合压力测试 / WP-X-04 ESG/治理扫描 / WP-X-05 展望监控）：评级、LGD、传染、SRI、展望等数值由 Python 确定性计算，非 LLM 即兴。
...
| **辅助件** | 报告模板（Type 1–19，Type 6/9/12 已归档）+ 可执行编排器（接 10 个编码引擎） | `dev/templates/` · `src/` |
...
可安装的发行包在 `version/v0.12.3-release/`（浏览/拷贝即用，包内 `INSTALL.md` 有分工具说明）；方法论源码在 `dev/`。
...
用户需求 → [路由: 四问 + 意图确认] → 《工作路径单》
        → [分析: 引擎文档 + 10 编码引擎] → [报告: 16 模板] → [质检: 质量门]
...
- ~~**v0.12.3**~~（已发布）：WP-X-04 ESG/治理扫描双引擎——GFR 信号规则库+一票否决 + ESG 事件映射链。
...
src/          可执行编排器 + 10 个编码引擎（pipeline.py · lgd_scorer.py · external_support_scorer.py 等）
scripts/      build_dist.py（dev/ → 发行包组装器）· consistency_check.py（一致性校验）
tests/        回归测试（517 项）
version/      当前可安装发行包 version/v0.12.3-release/（历史快照见 git 标签）
...
| **Extras** | report templates (Type 1–19, Type 6/9/12 archived) + executable orchestrator (10 coded engines) | `dev/templates/` · `src/` |
...
**13 industries · 16 work paths · 10 coded engines · 29 engine docs · 16 report templates**

### Highlights

- **Four-stage skill chain**: route → analyze → report → QA, with a mandatory intent-confirmation gate before execution.
- **8 reproducible coded-engine paths** (WP-M0-01 composite rating / WP-M0-02 LGD + external support / WP-M4-01 concentration / WP-M4-02 contagion / WP-M4-03 SRI / WP-M4-04 portfolio stress / WP-X-04 ESG/governance scan / WP-X-05 outlook): deterministic Python numbers, not LLM improvisation.
...
Request → [Router: 4 questions + intent confirmation] → Path Sheet
        → [Analysis: engine docs + 10 coded engines] → [Report: 16 templates] → [QA gates]
...
- ~~**v0.12.3**~~ (released): WP-X-04 ESG/governance dual engines — GFR signal rules + veto + ESG event mapping chain.
...
The installable package is at `version/v0.12.3-release/` (browse/copy and use; see `INSTALL.md` inside for per-tool setup); methodology source lives in `dev/`.

### Repository map

```
dev/          methodology & skill source (engine/ 29 docs · .claude/skills/ 4-stage chain · templates/ 16)
src/          executable orchestrator + 10 coded engines (pipeline.py · lgd_scorer.py · external_support_scorer.py et al.)
scripts/      build_dist.py (dev/ → release-package assembler) · consistency_check.py
tests/        regression tests (517)
version/      current installable package version/v0.12.3-release/ (history via git tags)
