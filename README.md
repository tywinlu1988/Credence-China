...
用户需求 → [路由: 四问 + 意图确认] → 《工作路径单》
        → [分析: 引擎文档 + 5 编码引擎] → [报告: 19 模板] → [质检: 质量门]
...
- **v0.10.1（规划中）**：M3 交易框架补全——盯市信号卡路径（WP-M3-01）激活（active 12/16），引擎缺口清零。
...
Request → [Router: 4 questions + intent confirmation] → Path Sheet
        → [Analysis: engine docs + 5 coded engines] → [Report: 19 templates] → [QA gates]
...
- **v0.10.1 (planned)**: M3 trading framework — market-watch signal-card path (WP-M3-01) activated (12/16 active); engine-gap cleared.
