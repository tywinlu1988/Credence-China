...
# T12.9 — (n) 生成的 AGENTS.md 保真：含防漂移铁律与编码引擎清单（口径区分：7 = 引擎模块数、
#          6 = wired 路径数——WP-M0-02 一路径接 lgd_scorer+external_support_scorer 双引擎），
...
def test_t12_9_generated_agents_md_fidelity(dist, builder):
    ...
    assert "7 个编码引擎" in readme, "generated README.md engine count stale"
    ...
    for pid in ("WP-M0-01", "WP-M0-02", "WP-M4-01", "WP-M4-02", "WP-M4-03", "WP-X-05"):
...
#          残留判定与构建器同源：直接复用 builder.INJECT_CSS_RE（单源，避免双处漂移）。


def test_dist_templates_css_injected(dist, builder):
    ...
    for f in files:
        ...
        assert not builder.INJECT_CSS_RE.search(text), f"{f.name} 有注入标记残留"
