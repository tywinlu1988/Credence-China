...
2. **Direct task, no path sheet.** If the user directly names a concrete task, read the core set — `engine/engine-overview.md` + `engine/dual-track-methodology.md` — plus any topic-specific doc the request names (e.g. `contagion-matrix.md`, `concentration-framework.md`, `lgfv-framework.md`).
3. **Vague / unrouted need.** If the need is ambiguous and no path sheet exists, first route through the `credit-analysis-router` skill, or ask the Q1–Q4 questions (role / object / depth / data) yourself to pick a path from `engine/work-path-registry.md`.
...
9. **条件专题阅读（强制）**：执行任何路径时，按标的情况追加阅读——城投标的 → `engine/lgfv-framework.md`（§五 政府信用定上限/平台定下限/意愿定落点）；金融债标的 → `engine/financial-bond-framework.md`；控股结构标的 → `engine/holding-company-framework.md`；单标的评级路径（WP-M0-01/M1-01/M1-02）定档前必经非信用风险叠加层检查 → `engine/non-credit-risk-overlay.md`（§八 叠加调整规则 + 信号触及章节）。

# Fixed Income Credit Analysis Engine v0.12.1-release
...
- Evaluating LGFV (城投债) credit quality through the LGFV framework → read `engine/lgfv-framework.md`
- Conducting ESG/governance risk scans and fraud detection → read `engine/esg-framework.md` and `engine/governance-fraud-risk.md`
- Performing LGD/recovery rate analysis for default scenarios → read `engine/lgd-recovery-framework.md`
- Assessing external support (government, parent company) impact on creditworthiness → read `engine/external-support-framework.md`
- Evaluating financial bonds → read `engine/financial-bond-framework.md`
- Analyzing holding companies → read `engine/holding-company-framework.md`
...
Full confidence/density model and gap-to-risk mapping: `references/mosaic-engine-architecture.md` (threshold single source: `engine/mosaic-engine.md`).
...
Defined but not implemented. Adapter contract (`query_bond_analytics` / `query_market_data` / `query_industry_benchmark`) and connection priority (CSV > REST API > MCP > DB): `engine/mosaic-engine.md`.

## 一票否决与评级上限 (Veto & Rating Ceiling — mandatory)

- **一票否决（one-shot veto）**：任一分析层触发一票否决条件时，该发行人评级**上限锁定为 CCC**，不得上调。每层否决条件见 `engine/industry-framework.md` §五。
- 评级映射一律采用官方 18 档表（`engine/dual-track-methodology.md` §六）；不得自造档位。
- 城投类标的：政府信用定上限、平台自身定下限、支持意愿定落点（`engine/lgfv-framework.md` §五）。
...
Full pyramid weights, Track-B thresholds, and the cross-comparison matrix: `references/industry-scoring.md` and `engine/dual-track-methodology.md`.
...
Full specification: `references/system-intelligence.md` and `engine/systemic-warning-framework.md`.
...
- **产出**：分析完成后输出《分析产物》（Analysis Artifact，schema 见 `engine/pipeline-contract.md` §2.2），`path_id` 承自路径单。
...
- **自动接续**：移交 report-builder 后无需用户指令，report → qa 自动接续；不得就"是否生成报告/做成哪种形式/是否质检/是否继续"询问用户——报告模板由 registry `templates` 字段决定，质检是链的必经终态。全链交互点预算见 `engine/pipeline-contract.md` §三「链式接续规则」。

## References

详情已下沉至 `references/`（单一事实源仍为 `engine/` 各引擎文档）：
...
- `.claude/skills/credit-analysis-router/SKILL.md` — 需求理解 / 路由层，产出《工作路径单》
- `engine/work-path-registry.md` — 工作路径注册表（路径单一事实源）
