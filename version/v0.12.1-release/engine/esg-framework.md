...
**版本**: v0.12.1-release | **日期**: 2026-07-10 | **状态**: 已发布 | **类型**: 非信用风险叠加层子模块
...
| 中小股东保护 | 仅分红异常（§2.3） | **新增**：分类表决、关联交易中小股东投票（分红政策权威定义见 GFR §2.3） |
...
> 与治理欺诈框架的分工：审计委员会频率、内审报告线、信披延迟、分红异常的权威定义见 [governance-fraud-risk.md](governance-fraud-risk.md) §2.3/§2.4（本框架不重复定义，直接引用其阈值）。
...
| 4.3.3 关键人员风险中的"CEO/CFO突然离职" | 本框架4.2（股权结构稳定性）和4.6（治理风险综合判断）提供治理视角 |
| 5.3.3 客户/供应商关系 | 本框架S3（产品质量安全风险）的客户流失传导路径 |

### 8.2 在 industry-framework.md 中的集成

在行业金字塔中，L1政策层应携带ESG敏感性标注（格式如下），标注内容由本框架附录A提供：
...
本框架产生的ESG信号在马赛克引擎的信号清单中标记为"ESG"类型，其定义为：
