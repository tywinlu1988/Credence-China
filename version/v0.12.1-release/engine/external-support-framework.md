...
|------|---------|---------|----------|------------|----------|------------|
| 母公司独立信用质量 | 使用本引擎对其他集团进行独立评估，排除子公司贡献后的实力 | 集团合并报表+母公司单体报表 | AAA/AA+ | AA/AA-/A+ | A/A-/BBB+/BBB | BBB-及以下或无评级 |
| 未质押资产规模 | 集团可用于融资/担保的剩余资产（操作化为 未质押资产/子公司债务 倍数） | 年报附注——受限资产披露 | ≥2（充裕覆盖子公司债务的2x+） | [1,2) | [0.5,1) | <0.5（核心资产均已质押） |
| 融资渠道多样性 | 银行授信+债券市场+股权融资 | Wind/评级报告/募集说明书 | 强：3种以上活跃渠道 | 中：2种活跃渠道 | 弱：渠道收缩至1-2种且不稳定 | 极弱：仅依赖单一渠道 |
| 经营活动现金流 | 集团自身经营产生的现金流（操作化为经营现金流利息覆盖倍数） | 合并现金流量表 | ≥1（持续为正且覆盖利息） | [0.3,1) | [0,0.3) | <0（持续为负） |
| 资产流动性 | 可变现资产（上市公司股权/金融资产等） | 年报——交易性金融资产+长期股权投资 | 强：充足且可变现 | 中：部分资产可变现 | 弱：可变现资产有限 | 极弱：大量非流动性资产（固定资产/在建工程） |

注：四档分值为 v0.12.1 裁决补充（强弱锚点为原文，中间档为裁决扩展），编码引擎 external_support_scorer 运行时解析本表。

### 4.4 战略投资者支持能力评估

| 指标 | 评估方法 | 数据来源 | 强（3分） | 中等（2分） | 弱（1分） | 极弱（0分） |
|------|---------|---------|----------|------------|----------|------------|
| 战投自身信用评级 | 使用本引擎或外部评级评估战投本身 | 外部评级/公开财报 | AAA/AA+ | AA/AA-/A+ | A/A-/BBB+ | BBB及以下 |
| 投资金额vs战投资产规模 | 战投对发行人的投资占其总资产的比例（小数口径：0.05=5%） | 战投财报——比例越低，支持越可持续 | <0.05 | [0.05,0.15) | [0.15,0.30) | ≥0.30 |
| 投资承诺的法律约束力 | 增资协议/担保函/支持函的法律效力 | 上市公司公告/协议文本 | 强：担保/差额补足等强法律约束承诺 | 中：有法律效力的支持函 | 弱：意向性支持安排 | 极弱：无书面承诺 |
| 锁定期/退出安排 | 战投锁定期的时长和退出条款（锁定期单位：年） | 投资协议（需关注公开披露中的退出条款） | ≥3 | [2,3) | [1,2) | <1 |

注：四档分值为 v0.12.1 裁决补充（本表四档列整列新增，无原文强弱锚点），编码引擎 external_support_scorer 运行时解析本表。
...
典型政策信号对"支持能力/支持意愿"的映射表（归并自 qualitative-analysis.md §3.4.3，v0.11.2 起本表为单一事实源）：
