...
根据《传染理论基础》(v0.8.5-release)定义的 **6 种分析范式 + 1 个特殊类别（LGFV）**，将13个行业映射至对应范式。同一范式内的行业因共享相同的风险驱动因子、融资特征和市场叙事逻辑，**先天性具有更高的传染耦合度**。
...
> **注意**：confidence 和 historical_cases 在当前 v0.8.5-release 的 SRI/集中度计算中未被量化消费；它们用于人工审阅和未来版本扩展。
...
*本文档是《传染理论基础》(v0.8.5-release)的操作化延伸，两者需配合使用。*
