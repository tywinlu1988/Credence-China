...
注：Δ 为带符号调整（负值=降低 LGD），本公式为加法式（v0.12.0 修正原文减号笔误，与 Δ 语义及 §5.1 先验对齐）。
...
LGD 作为**独立模块**输出而非嵌入评级字符串（既定决策），原因：

1. 保持对现有 PD 评级框架的向后兼容
2. LGD 是针对债项而非发行人的属性，同一发行人的不同债券可能有不同 LGD
3. 双轨方法论的核心输出（评级+信号+完备性报告）在每个债券层面可附加 LGD 信息

评级映射的权威定义见 [dual-track-methodology.md](dual-track-methodology.md) §六；LGD 等级与评级的组合表示（如 AAA/LGD2）按上述独立模块原则附加。
