...
| **股权融资失败/IPO终止** | 股权融资失败或IPO被终止（资本市场信心不足） | 🟠 重大 | 交易所公告（L6） |
| **分红异常** | ① 连续3年盈利但不分红 ② 分红率突然大幅下降但无合理解释 | 🟡 中 | 年报利润分配方案 |

注："股权融资失败/IPO终止"行归并自 qualitative-analysis.md §4.4 跨行业通用红旗信号，v0.11.2 起本节为单一事实源。
...
| **交易所问询函/监管关注** | 收到交易所问询函或监管关注函（监管已注意异常） | 🟠 关注 | 交易所公告（L6） |
| **被申请仲裁/诉讼频率骤增** | 被申请仲裁/诉讼的频率骤增（反映经营纠纷增多，区别于上表按标的金额判断的重大诉讼） | 🟠 关注 | 裁判文书网（L6） |
| **净资产为负/资不抵债** | 年报净资产为负（事实上的D级状态） | 🔴 一票否决 | 年报（L2） |

注：上表末三行（交易所问询函/诉讼频率骤增/净资产为负）归并自 qualitative-analysis.md §4.4 跨行业通用红旗信号，v0.11.2 起本节为单一事实源。
...
**通用红旗的叠加效应（归并自 qualitative-analysis.md §4.4）：**
...
> **口径说明**：本规则按**通用红旗条数**计数触发，与上表按**信号强度**（中/高强度信号数）分级的口径**并行适用**。归并自 qualitative-analysis.md §4.4（已归档），v0.11.2 起本节为单一事实源。
...
| **实控人/高管涉及刑事犯罪** | 实控人或高管因行贿/受贿/挪用资金/职务侵占被刑事立案或判决，或公告确认被拘留/调查/取保候审 | **强** | 裁判文书网、检察院公告、媒体报道 | **可观测**——刑事程序公开 |
...
non-credit-risk-overlay.md 的"相关内容"将本模块引用为操作风险子集来源（欺诈信号+管理层治理红旗+系统故障/合规违规/关键人员风险扩展）。

操作风险信号（含欺诈+系统+合规+人员）在双轨方法论中作为轨道A L4财务层的交叉补充信号——这些信号的存在会降低财务层评分的可信度（欺诈信号明显时）或增加下行风险（合规/人员信号明显时）。
