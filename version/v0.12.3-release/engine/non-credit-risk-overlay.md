...
**版本**: v0.12.3-release | **日期**: 2026-07-10 | **定位**: 信用金字塔基准评级之上的独立调整层
...
风险管理标准审计对照Basel新资本协议和COSO ERM 2017框架，识别出引擎当前覆盖的7类Basel风险类型中存在显著缺口：
...
信号清单的权威定义见 [governance-fraud-risk.md](governance-fraud-risk.md) §7.3.3（单一事实源），本处不复制。以下为 GFR 未覆盖的本模块独有补充信号：
...
信号清单的权威定义见 [governance-fraud-risk.md](governance-fraud-risk.md) §8.2（单一事实源），本处不复制。以下为 GFR 未覆盖的本模块独有补充信号：
...
信号清单的权威定义见 [governance-fraud-risk.md](governance-fraud-risk.md) §9.2（单一事实源），本处不复制。以下为 GFR 未覆盖的本模块独有补充信号：
...
信号清单的权威定义见 [governance-fraud-risk.md](governance-fraud-risk.md) §2.4（单一事实源），本处不复制。以下为 GFR 未覆盖的本模块独有补充信号：
...
非信用风险叠加在双轨方法论决策流程中的位置：步骤4（评级映射，含外部支持调整）之后、步骤5（交叉对撞）之前。
...
本模块定义的非信用风险缺口与马赛克引擎"缺口→风险映射表"（5.2节）的对应映射如下：
...
本模块输出的非信用风险信号在完备性评估层（5.1节）的信号密度条形图中作为独立维度展示，更新后形态如下：
...
本模块输出的非信用风险事件信号由展望监控框架按以下频率与触发条件监测：
...
展望监控框架的"触发重评条件"在既有 P0 监控项之外，还包括"非信用风险叠加后评级下调"。

### 10.4 在 industry-framework.md 中的集成

各行业金字塔的 L2 技术层信号与本叠加层战略风险维度（strategic_risk）的衔接规则：

> ⚠️ L2技术层信号与战略风险（非信用风险叠加层）的衔接：
> 当L2技术层评分涉及的"技术路线被淘汰"风险达到临界点时，
> 该信号同步传递给非信用风险叠加层（strategic_risk维度）。
> 详见本文档 6.2节。
