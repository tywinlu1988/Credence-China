...
**版本**: v0.12.3-release | **日期**: 2026-07-10 | **状态**: 已发布
...
11. [SRI 时间序列追踪](#十一sri-时间序列追踪)
12. [组合级 SRI](#十二组合级-sri)
13. [SRI 压力测试](#十三sri-压力测试)
14. [SRI × 传染升级联动](#十四sri--传染升级联动)
15. [局限性声明](#十五局限性声明)
16. [附录](#十六附录)
...
评分→评级的权威映射见 [dual-track-methodology.md](dual-track-methodology.md) §六（18 档，单一事实源，本处不复制表体）。行业健康状态按评级档分带：
...
传染力系数由 `src/contagion_engine.py` 运行时从[传染矩阵](contagion-matrix.md)派生（执行的单一事实源）：取矩阵 §二 各行业**行合计**（对外传染力总分），除以 13 行业行合计均值：

```
传染力系数(行业 i) = 行合计(i) / mean(全部 13 行业行合计)
```

- 系数 > 1.0 = 传染力高于均值（权重上浮）；< 1.0 = 低于均值（权重下调）
- 行合计前三名为**超级传染者**，在 SRI 计算中获得更高权重（当前排名见传染矩阵 §5.5）

> 本文档不保留逐行业系数静态表——任何静态副本都会与矩阵派生值漂移（v0.8.2 教训）。
...
以半导体行业为例，假设信用债存量占比为1.5%、传染力系数为1.29（示意值——实际系数由 contagion_engine 按 §2.3.1 公式运行时派生，本文档不固化）：

```
半导体行业权重百分比 = 1.5% × 1.29 ≈ 1.94%
...
步骤3：计算行业权重百分比
  ├── 信用债存量权重 ← 市场数据
  ├── 传染力系数 ← contagion_engine 从传染矩阵(contagion-matrix.md)运行时派生
...
| 传染力系数 | contagion_engine 从[传染矩阵](contagion-matrix.md)运行时派生（§5.5） | 版本更新时调整 |
...
| **历史验证** | 2020年Q1疫情冲击时，全部13行业同时承压，SRI显著上升（§7回测测算约1.15，🟠警惕） |
...
各行业权重由以下公式链计算（逐行业静态数值表不再保留，由 `src/contagion_engine.py` + `src/sri_calculator.py` 运行时计算）：

```
原始权重(行业 i) = 信用债存量占比(i)〔§2.3.2〕 × 传染力系数(i)〔§2.3.1〕
归一化权重 = 原始权重 / Σ(原始权重)，确保 Σ(权重) = 100%
```

**注：** 房地产行业不在引擎的13行业覆盖范围内，但其信用债存量约5%。在SRI计算中，这部分可归入相关行业（如城投债通过土地财政关联）或作为"其他行业"单独处理。当前版本将房地产信用债按区域信用债加回城投债权重。

**数据来源：** 信用债存量占比基于Wind资讯2026年Q2信用债市场统计（含企业债、公司债、中票、短融、定向工具），覆盖银行间和交易所市场。传染力系数由 contagion_engine 从传染矩阵运行时派生。房地产存量占比5%仅供参考，不纳入SRI计算。数据中心/算力基建的信用债存量并入城投债口径（园区/基建关联），不单列。

**实际简便处理：** 由于城投债的权重占比过高，在实际计算中推荐使用**截断权重**：
...
| 设计 | 说明 |
|------|------|
| **传染力系数折中** | 城投债是超级传染者#2(23分)，低于半导体(25分)，传染力系数（由 contagion_engine 运行时派生，见 §2.3.1）在超级传染者中居中——防止权重叠加过大 |
...
### 11.1 SRIReading 数据结构

SRI 时间序列追踪基于 SRIReading 数据点序列，每次计算保留一个完整快照：

| 字段 | 类型 | 说明 |
|------|------|------|
| timestamp | datetime | 计算时点 |
| sri_value | float | SRI 读数 |
| thermometer | str | 温度计等级（normal/watch/alert/danger） |
| risk_scores | dict[str, float] | 各行业风险得分 |
| weights | dict[str, float] | 各行业归一化权重 |
| top_contributors | list[str] | 前 3 大贡献行业 |

### 11.2 数据要求

有效趋势分析需要至少连续 4 期读数（季度频率约 12 个月），推荐 8 期以上（24 个月）。读数间隔应均匀，缺失期直接跳过，不进行插值填补。

### 11.3 compute_trend 算法

趋势判断使用线性回归斜率法：

```
斜率 k = Σ((t_i - t̄)(sri_i - srī)) / Σ((t_i - t̄)²)
...
阈值 ±0.05 跨 8 期（约 2 年）对应约 0.4 SRI 的变化——足以在 2 年内从🟢上升至🟠警惕级别，属有意义的变化信号。

### 11.4 tipping_points 事件

| 事件 | 触发条件 | 含义 |
|------|---------|------|
| 拐头向上 | 连续 2 期上升且斜率 > +0.05 | 风险开始积累 |
| 拐头向下 | 连续 2 期下降且斜率 < -0.05 | 风险开始消退 |
| 温度跃升 | SRI 跨过 0.5/1.0/1.8 阈值 | 温度计升级 |
| 温度回落 | SRI 回落至下一阈值以下 | 温度计降级 |

### 11.5 局限

趋势分析基于历史数据，无法预测突发事件。短期（≤4 期）趋势具有较大不确定性，不应作为独立决策依据。tipping_points 为回顾性标识，非前瞻性预测。
...
组合级 SRI 使用组合实际持仓权重替换市场固定行业权重：

```
组合 SRI = Σ(risk_score_i × holdings_i × coeff_i) / Σ(holdings_j × coeff_j)

其中：
  risk_score_i  = 行业 i 的风险得分（与市场 SRI 相同算法）
  holdings_i    = 组合中行业 i 的持仓占比（归一化至 100%）
  coeff_i       = 行业 i 的传染力系数（来自传染矩阵）
```

归一化分母确保组合 SRI 与市场 SRI 同尺度（0-3+），可直接对比。

### 12.2 与市场 SRI 的区别

| 维度 | 市场 SRI | 组合 SRI |
|------|---------|---------|
| 行业权重 | 固定：信用债存量×传染力系数 | 可变：组合实际持仓 |
| 适用范围 | 全市场系统性风险 | 特定组合风险暴露 |
| 更新频率 | 季度（权重随市场更新） | 按需（调仓时重算） |
| 解读 | "市场整体有多危险" | "我的组合有多危险" |

### 12.3 适用场景

| 场景 | 说明 |
|------|------|
| 组合合规检查 | 组合 SRI > 1.0（🟠）时自动收紧集中度限额 |
| 多组合对比 | 比较不同组合的 SRI 读数，识别最高风险组合 |
| 调仓模拟 | 假设调仓→新持仓→新 SRI→效果量化 |
| 归因分析 | 组合 SRI 变化分解为调仓效应与市场信号变化 |
...
| 参数 | 温和情景 | 严重情景 | 极端情景 |
|------|---------|---------|---------|
| 受冲击行业数 | 1 个 | 2 个 | 13 个（全部） |
| 受冲击行业 | 光伏/储能 | 光伏/储能、城投债/LGFV | 全部 13 行业 |
| 轨道 A 下调 | -1 分 | -2 分 | -3 分 |
| 展望调降 | 不变 | 受冲击行业→负面 | 全部→负面 |
| 轨道 B 跳升 | 不变 | 受冲击行业升 1 级 | 全部升 1-2 级 |
| 升级因子 | 无 | 信用事件 | 恐慌 + 信用事件 + 高杠杆 |

> **注（引擎因子名映射）**：表中升级因子为描述性简称；编码引擎（sri_calculator / contagion_engine）的合法因子名为「市场恐慌 / 监管真空 / 高杠杆 / 信息不对称 / 年末效应」（传染矩阵 §6.2 枚举）。对应关系：「信用事件」→「市场恐慌」（信心崩塌 S 型传染的代表因子），「恐慌」=「市场恐慌」；编码侧 PREDEFINED_SCENARIOS 原「流动性危机」→「高杠杆」（流动性挤兑 L 型）、「政策突变」→「监管真空」（Fix R1 语义 1:1 映射，与 src/sri_calculator.py 注释互锚）。

#### 温和情景

参数设定：模拟单一行业（光伏）基本面小幅恶化——如产能出清慢于预期，轨道 A 从 4.5-5.5 降至 3.5-4.5 区间，不触发升级因子，不影响其他行业。

压力后预期：SRI 上升约 0.05-0.10，温度计等级不变或上移至🟡关注上沿。

触发条件：组合对光伏敞口 > 15% 且光伏已处于关注状态。

#### 严重情景

参数设定：模拟两个核心行业（光伏 + 城投债）同时承压——光伏基本面恶化叠加城投化债推进中某弱区域出现信用事件，触发"信息不对称"升级因子。两行业轨道 A 各降 2 分，展望调为负面，轨道 B 信号升 1 级。

压力后预期：SRI 上升约 0.20-0.40，温度计大概率升级至🟠警惕。

触发条件：组合对光伏 + 城投债联合敞口 > 30%，或两行业同时出现负面信号。

#### 极端情景

参数设定：模拟全市场系统性冲击——类似 2020 年 Q1 疫情的全面恶化。13 行业轨道 A 统一下调 3 分（约等于全行业评级下调 2-3 子级），展望全部负面，轨道 B 跳升 1-2 级，恐慌 + 信用事件 + 高杠杆三升级因子同时触发（传染矩阵 §6.3 → 全域链路 +1~+2）。

压力后预期：SRI 升至 1.5-2.5，温度计进入🔴危险。

触发条件：外部环境出现明显系统性冲击信号（疫情/地缘冲突/监管巨变）。

### 13.2 stress_test 算法流程
...
步骤 2：施加冲击
  ├── 按"受冲击行业"列表调降轨道 A（降幅参数）
  ├── 按"展望调降"规则更新展望方向
  ├── 按"轨道 B 跳升"规则更新轨道 B 等级
  └── 按"升级因子"列表调用 apply_escalation()

步骤 3：重算各行业风险得分
  └── industry_risk_score() 逐一计算

步骤 4：重算传染力系数
  └── contagion_coefficients(加压后矩阵)

步骤 5：计算压力后 SRI
  └── portfolio_sri(新风险得分, 持仓权重, 新系数)

步骤 6：对比输出
  └── 基准 vs 压力 → delta → 温度计变化 → 贡献分解
```
...
| 输出字段 | 说明 |
|---------|------|
| baseline_sri | 压力前 SRI 读数 |
| stressed_sri | 压力后 SRI 读数 |
| delta | stressed_sri - baseline_sri |
| baseline_level | 压力前温度计等级 |
| stressed_level | 压力后温度计等级 |
| top_drivers | 风险得分变化最大的前 3 行业 |

| delta 范围 | 解读 |
|-----------|------|
| < 0.10 | 压力耐受——组合对冲击不敏感 |
| 0.10-0.30 | 压力关注——需检查暴露行业 |
| ≥ 0.30 | 压力警惕——建议主动降低敞口 |
| 温度计升级 | 触发集中度限额自动收紧 |
| 跨两级升级 | 启动流动性应急预案 |
...
传染矩阵强度跳升（§6.2 跳升规则）
    ↓
contagion_coefficients 变化（部分行业上升）
    ↓
SRI 权重再分配（高系数行业权重被动上升）
    ↓
SRI 重算——读数变化可能跨温度计阈值
    ↓
联动反馈——SRI 上升→温度计升级→传染矩阵全域 +1（§3.4）
```

### 14.2 linked_sri 算法

```
linked_sri(industries, holdings, matrix, escalation_factors)

输入：
  - industries: list[IndustryInput]——组合涉及的行业
  - holdings: dict[str, float]——行业持仓权重
  - matrix: 13×13 传染矩阵
  - escalation_factors: list[str]——已触发的升级因子

流程：
  1. apply_escalation(matrix, factors) → 加压后矩阵
  2. contagion_coefficients(pressured_matrix) → 新系数
  3. portfolio_sri(industries, holdings, new_coeffs) → 压力后 SRI
  4. 对比基准（无升级因子）→ delta → explanation
...
### 14.3 explanation 生成规则

| 条件 | 模板 |
|------|------|
| delta < 0.05 | "升级因子对 SRI 影响有限（+{delta:.2f}），无需额外行动" |
| 0.05 ≤ delta < 0.15 | "升级因子触发后 SRI 上升 {delta:.2f}，主因 {top_factor}" |
| delta ≥ 0.15 | "升级因子显著推高 SRI（+{delta:.2f}），建议启动压力测试" |
| 温度计升级 | "SRI 从 {from_level} 升至 {to_level}，集中度限额自动收紧" |

### 14.4 局限

升级因子集需与传染矩阵 §6.2 同步维护——新增或调整升级因子时 escalation_factors 参数需同步更新。当前算法假设各因子独立叠加，未建模非线性交互（参见传染矩阵 §6.3 协同效应）。
...
| v0.8.4-release | WP-X-05 展望监控激活接线（本框架与阈值无变更） |
| v0.8.5-release | WP-M0-01 旗舰聚合接线（本框架与阈值无变更） |
| v0.8.6-release | 体验与引导强化（本框架与阈值无变更） |
| v0.9.0-release | 广度收口与模板全域升级（本框架与阈值无变更） |
| v0.9.1-release | 四段链协议加固（本框架与阈值无变更） |
| v0.9.2-release | 发行包 pipeline 可导入热修（本框架与阈值无变更） |
| v0.9.3-release | 文档一致性修复（本框架含 §2.1 表头 12档→18档、§11.3 未来行重编号至 v0.10.3） |
| v0.9.4-release | 小问题清扫（本框架与阈值无变更） |
| v0.9.5-release | 防漂移约束体系（本框架与阈值无变更） |
| v0.9.6-release | 包内 AGENTS.md 生成器纠偏热修（本框架与阈值无变更） |
| v0.10.0-release | 扫尾清单机制化：promote 自动追加三处版本历史行与 README 路线图翻转 + roadmap_freshness 门禁（含同版本防重/失锚提示）（本框架与阈值无变更） |
| v0.10.1-release | M3 交易框架补全：m3-trading-framework 编排层文档 + Type 19 L0 信号卡模板 + WP-M3-01 激活（active 12/16）+ 报告独立性修复（CSS 内联/交付硬化/索引页/qa 8 项）（本框架与阈值无变更） |
| v0.10.2-release | partial 激活扫尾：WP-M0-02/WP-M1-02/WP-M4-04/WP-X-04 四条全 active（16/16 零 partial）+ 逐路径走查（本框架与阈值无变更） |
| v0.10.3-release | SRI 深化四件套：时间序列追踪/组合级 SRI/压力测试/传染联动 + systemic §11.3 规划落地 |
| v0.10.4-release | 沙箱审计修复：生成器计数动态化/多事实源一致性/入口文档充实/状态行系列完结（v0.10.x 系列收尾）（本框架与阈值无变更） |
| v0.11.0-release | 硬重复清零：引擎文档副本指针化+冲突裁决+施工残留清理+Type12归档+单源回归锁（本框架与阈值无变更） |
| v0.11.1-release | 模板装配化：CSS构建期注入+跨域嵌入降级+Type9/6并入Type8/1（本框架与阈值无变更） |
| v0.11.2-release | 孤儿裁决：定性/定量全量归并+归档+条件阅读表+零孤儿锁（本框架与阈值无变更） |
| v0.12.0-release | 编码引擎扩线首波：WP-M0-02双引擎（LGD+外部支持）接线（本框架与阈值无变更） |
| v0.12.1-release | 还债版：集团/战投capacity量化表补建+技术债清零（本框架与阈值无变更） |
| v0.12.2-release | WP-M4-04压力测试引擎：M4集群全编码化（本框架与阈值无变更） |
| v0.12.3-release（当前） | WP-X-04 ESG/治理扫描双引擎（本框架与阈值无变更） |
| v0.10.3-release | SRI 时间序列追踪——SRIReading 数据结构、compute_trend 算法、tipping_points 事件（§十一） |
| v0.10.3-release | 组合级 SRI——基于实际持仓权重的 portfolio_sri 算法（§十二） |
| v0.10.3-release | SRI 压力测试——三档情景参数表、stress_test 算法流程（§十三） |
| v0.10.3-release | SRI×传染升级联动——linked_sri 算法、explanation 生成规则（§十四） |
...
*本文档应与《双轨分析方法论》(v0.12.3-release)、《传染矩阵》(v0.12.3-release)、《五维集中度分析框架》(v0.12.3-release)、《展望监控框架》配合使用。系统性预警框架是引擎M4组合风控层的顶层仪表盘，为分散的行业信号提供统一的系统性风险读数。*
