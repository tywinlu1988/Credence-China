...
根据《传染理论基础》(v0.12.3-release)定义的 **6 种分析范式 + 1 个特殊类别（LGFV）**，将13个行业映射至对应范式。同一范式内的行业因共享相同的风险驱动因子、融资特征和市场叙事逻辑，**先天性具有更高的传染耦合度**。
...
> **注意**：confidence 和 historical_cases 在当前 v0.12.3-release 的 SRI/集中度计算中未被量化消费；它们用于人工审阅和未来版本扩展。
...
> 行合计权威值见 §5.5（contagion_engine 运行时从 §二 矩阵派生），本表为金融属性原则的示意。
...
**特别说明：** 监管政策真空对城投债相关对的影响最大(最大跳升+1~+2)，对其他非城投相关对影响有限(+0~+1)。原因：监管表态直接决定区域信用走向（见 [contagion-theory.md](contagion-theory.md) §5.2.2）。
...
| 文档 | 路径 | 修改内容 |
|------|------|---------|
| contagion-theory.md | engine/contagion-theory.md | 在6.3节"与现有引擎文档的集成"中增加指向本矩阵的链接 |
| industry-framework.md | engine/industry-framework.md | 在框架模板中增加"传染暴露"章节占位 |
| paradigm-*.md (6个范式文档) | engine/paradigm-*.md | 每个范式文档的"特殊风险"章节引用本矩阵 |
| output-layered-framework.md | engine/output-layered-framework.md | 在M4组合风控部分增加传染矩阵引用 |
| concentration-framework.md | engine/concentration-framework.md | 在§9.2集成表中增加本矩阵引用 |
| systemic-warning-framework.md | engine/systemic-warning-framework.md | 在§10.3集成表中增加本矩阵引用 |
...
*本文档是《传染理论基础》(v0.12.3-release)的操作化延伸，两者需配合使用。*
