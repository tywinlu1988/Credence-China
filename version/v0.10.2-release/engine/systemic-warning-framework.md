...
| v0.10.3（规划） | 增加SRI时间序列追踪（绘制SRI历史曲线，识别趋势和拐点） |
| v0.10.3（规划） | 引入SRI与传染矩阵升级因子的实时联动（当升级因子触发时自动调整SRI读数） |
| v0.10.3（规划） | 增加组合级别的SRI计算（基于组合实际持仓权重替代行业权重），实现真正的组合系统性风险评估 |
| v0.10.3（规划） | 引入SRI压力测试（输入假想冲击→输出压力后的SRI温度计），与M4组合风控深度集成 |
