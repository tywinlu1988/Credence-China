...
## Intake Protocol（四问，渐进式单问制）

**发问节奏（强制）**：每次只问**一个**问题，附 A/B/C/D 快捷选项供勾选作答；得到回答后再发下一问。**绝不在一条消息里列出全部四问**，不得把四问作为一张清单抛出要求一次性回答。用户主动一次提供多项信息时照单收纳（不强迫重复作答），但发问方必须保持单问制。

**已覆盖即跳过**：用户首条消息已明确的信息（角色/对象/深度/数据）不再追问，只补问缺失项；四项俱足时直接进入 Step 5。

- **Q1 角色**：您以什么身份决策？A. M0 审贷 / B. M1 投资 / C. M2 承销 / D. M3 交易 / E. M4 风控 / F. M5 融资 / G. 不确定（按问题特征推断后向用户确认）。
- **Q2 对象**：分析对象是什么？A. 单发行人 / B. 债券组合 / C. 行业 / D. 全市场 / E. 方法论建设或引擎验证。
- **Q3 深度**：需要多深的产出？A. L0 快速信号 / B. L1 决策快照 / C. L2 深度报告 / D. 专项报告。
- **Q4 数据**：用什么数据？A. 仅公开数据（Mode A）/ B. 用户显式提供外部数据源（CSV/API/MCP → Mode B）。
...
quality_gates: []           # "规则名 (engine/<doc>.md §节)"
notes: ""
```
...
- **partial 路径如实告知**：推荐到 🟡 partial 路径（如 WP-M0-02、WP-M1-02、WP-M4-04、WP-X-04）时，必须说明该路径当前的完备度边界，并给出可替代的 active 路径，不得伪造能力。
- **不脑补**：意图要素缺失时给候选不给结论（如"角色不明"→ 给 2 条候选路径请选，不擅自定）；用户表述与注册表全部路径都不匹配时，如实说明并给出最接近的 1-2 个选项，禁止强行套路径。未经 Step 5 确认直接执行属违规。不推荐引擎文档之外的分析框架、维度或报告形式（防漂移）。
...
- **下游（REQUIRED NEXT SUB-SKILL）**：`fixed-income-credit-analysis` —— 《工作路径单》产出后即移交该 skill 执行分析；四段链产物契约见 `engine/pipeline-contract.md`。
- **全链自动接续**：经 Step 5 确认后，analysis → report → qa 各段**自动接续执行**，无中途确认点。全链仅 3 个合法用户交互点（Step 5 意图确认 / Mode B 数据缺口澄清 / 质检 fail 且一轮自愈失败），规则单一事实源为 `engine/pipeline-contract.md` §三「链式接续规则」。

## References

- `references/work-paths.md` — 工作路径路由视图（精简表，标注状态与一句话触发特征）
- `engine/work-path-registry.md` — 工作路径注册表（单一事实源）
