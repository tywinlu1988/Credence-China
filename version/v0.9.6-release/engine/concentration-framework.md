...
> **跨文档消费说明**：集中度框架在引用传染矩阵时，仅消费每个单元格的 **`intensity`** 和 **`direction`** 两个字段，用于识别传染强度与判断单向/双向关系。`type`、`confidence` 和 `historical_cases` 在当前 v0.9.6-release 的集中度评分中不进入量化计算。
...
*本文档应与《传染矩阵》(v0.9.6-release)和《财务深度分析》(v0.9.6-release)配合使用。集中度分析框架是M4组合风控层的核心组件，需与行业金字塔(M1-M2)、双轨分析(M3)形成完整的风控闭环。*
