...
def _version() -> str:
    ...
    return m.group(1) if m else "v0.12.1-release"


def _template_range() -> str:
    """在役模板区间的诚实表述（顶层非递归 glob；archive/ 历史留档不计入）。
    编号族最大号 + 在役份数，如 "Type 1–19，共 18 份"——存在归档缺口时不伪装连续区间。
    T12.9 保真锁同源真值。"""
    nums = sorted(
        int(re.fullmatch(r"template-type(\d+)\.html", f.name).group(1))
        for f in (DEV / "templates").glob("template-type*.html")
    )
    return f"Type 1–{nums[-1]}，共 {len(nums)} 份"
...
| `credit-report-builder` | 把完成的信用分析装配为交付报告（选模板 {_template_range()}、映射 L0/L1/L2 层、装配仪表盘）；需上游分析产物，自身不做分析 | `.claude/skills/credit-report-builder/SKILL.md` |
...
1. **禁自造报告形式**：交付报告一律使用 `templates/` 中工作路径注册表为该路径指定的模板（{_template_range()}），禁止自行设计版式或"参考模板风格自制"；无可用模板时如实告知，不自创。
...
`engine/work-path-registry.md` 是路由单一事实源：**16 条工作路径（16 条 active / 0 条 partial / 0 条 planned）**。router 据此把模糊需求路由到具体工作路径；全部路径均已激活可直接使用。
...
- `src/` — 可执行编排器与 7 个编码引擎（composite_scorer 旗舰聚合、lgd_scorer LGD 评估、external_support_scorer 外部支持评估、concentration_scorer 五维集中度、contagion_engine 传染矩阵、sri_calculator SRI、outlook_engine 展望监控）
- `adapters/` — 按工具的深度适配说明
"""
