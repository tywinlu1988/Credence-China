...
def _version() -> str:
    ...
    return m.group(1) if m else "v0.10.3-release"
...
`engine/work-path-registry.md` 是路由单一事实源：**16 条工作路径（16 条 active / 0 条 partial / 0 条 planned）**。router 据此把模糊需求路由到具体工作路径；推荐到 partial 路径时须如实告知完备度边界并给出可替代的 active 路径。
