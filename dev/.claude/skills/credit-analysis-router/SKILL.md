...
- **全链自动接续**：经 Step 5 确认后，analysis → report → qa 各段**自动接续执行**，无中途确认点。全链仅 3 个合法用户交互点（Step 5 意图确认 / Mode B 数据缺口澄清 / 质检 fail 且一轮自愈失败），规则单一事实源为 `dev/engine/pipeline-contract.md` §三「链式接续规则」。
