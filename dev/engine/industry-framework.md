...
本小节内容归并自 qualitative-analysis.md §4.3 增量（红旗处理规则与列语义），v0.11.2 起本小节为单一事实源。各行业特有红旗信号清单随 qualitative-analysis.md 全量归档（见 dev/engine/audits/qualitative-analysis.md §4.3；一票否决级条件以指针引用本节主表，不复制否决条件文本）；本节只定义非否决信号的元数据语义与处理规则。
