...
> **注**：融资成本变动为利率加点（bp），利息支出放大系数 = 1 + 加点/基准融资利率（E.9 口径）；基准利率缺失时按 ×(1+加点) 简化（低估冲击）。
...
归并自 quantitative-analysis.md §6.2-6.5（华晨回溯案例精简为结论指针），v0.11.2 起本节为单一事实源；quantitative-analysis.md 已全量归并并归档至 dev/engine/audits/。

**定位：** E.1-E.9 的场景敏感性针对发行人**财务基本面**（三表传导），本节补充**债券市值维度**——在无法获取修正久期/凸性、无法实施蒙特卡洛模拟的公开数据约束下，估算利率/利差冲击对债券市场价值的影响。适用范围限定为固定利率非含权纯债；含权债/可转债仅作方向性参考。
...
**局限标注：** 1) 以上为线性近似，不含凸性修正——利率变化越大，误差越大；2) 含权债不能使用此方法——久期被期权调整，实际敏感度低于近似值。
...
在无法实施蒙特卡洛模拟的约束下，使用**情景加权法**：对每个因子独立计算影响，假设因子间独立（简化假设），ΔP_total = Σ(ΔP_i × S_i)（ΔP_i = 第i个因子冲击下的价格变化，S_i = 该因子在情景中发生的概率，0-1）。**缺陷：** 忽略因子相关性——压力情景下因子相关性会系统性上升，须标注"独立假设可能低估总影响，误差约10-20%"。
...
**定量估算示例**（3年期中票，YTM=3.5%，近似久期=2.90）：轻度承压（总YTM冲击+100bp）→ -2.90%；中度（+200bp）→ -5.80%；严重（+400bp）→ -11.60%；极端尾部（+700bp）→ -20.30%。局限：利率+利差独立假设在压力时期不成立（实际冲击可能更大）；未考虑凸性（方向性正确但量级有偏）；未考虑交易对手风险、追加保证金等非线性影响。

#### E.10.3 尾部情景构建与文档模板

尾部风险情景的目标不是"预测"，而是**暴露出组合对特定极端事件的脆弱性**。三种构建方法：**历史重演**（选取历史上最严重的信用冲击事件，如"2015年股灾+债灾"/"2020年永煤冲击"——可行性高，数据可得）；**假设性最差**（本企业可想象的最坏情况，如"核心客户破产+再融资冻结+评级下调至D"——可行性高，依赖行业判断）；**压力因子叠加**（多个低概率因子同时发生，如"利率200bp+利差400bp+股价跌50%"——可行性中，因子相关性不可知）。
...
#### E.10.4 华晨案例回溯验证（结论指针）与经验教训

**回溯验证结论**（17华汽05，分析日2018-12-31，T-22个月回溯验证案例）：方向性准确——"宝马剥离情景"预判母公司信用重定价（YTM 7.5%→12.0%、价格影响-10.8%），现实为2020-10首次违约、无担保债权回收率约30-40%（预测40%，基本准确）；但违约前二级市场YTM实际到20%+——低估了恐慌程度，流动性溢价未被充分计入。完整情景构建与数据表已随 quantitative-analysis.md 归档（dev/engine/audits/quantitative-analysis.md §6.5）。
