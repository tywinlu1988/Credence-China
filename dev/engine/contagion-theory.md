...
| 升级因子 | 基础状态（1-2级传染） | 升级状态（3-4级传染） | 极端状态（5级传染） | 可观测指标 |
|---|---|---|---|---|
| **市场恐慌情绪** | VIX<20，信用利差正常 | VIX 20-35，利差走阔30-50bp | VIX>35，利差翻倍 | VIX指数、信用利差、CDS价格 |
| **监管政策真空** | 监管明确表态维稳 | 监管沉默/"不便评论" | 监管宣布"让市场自己解决"/不救助 | 监管会议纪要、官员采访、政策文件 |
| **高杠杆环境** | 市场整体杠杆率<80%（以债券质押式回购规模衡量） | 杠杆率80-110% | 杠杆率>110%+部分机构强制平仓 | 质押式回购规模、基金杠杆率、券商自营杠杆率 |
| **信息不对称** | 发行人与市场保持沟通 | 发行人沉默/公告模糊 | 发行人完全失联/董事失联 | 公告频率、电话会出席情况、管理层可及性 |
| **年末/季末效应** | 非季末时段 | 季末前1-2周 | 年末最后两周 | 日历日期+银行间流动性 |

> 各因子的触发条件、强度跳升规则与协同系数的单一事实源为 [contagion-matrix.md](contagion-matrix.md) §六；本表只保留状态刻度（VIX 三级、杠杆率分档等）与可观测指标。
...
归并自 quantitative-analysis.md §5.1-5.5（§5.5.1 相关性统计量置信区间规范的单一事实源为 [dual-track-methodology.md](dual-track-methodology.md) 附录.2.6，不在本节重复），v0.11.2 起本节为单一事实源；quantitative-analysis.md 已全量归并并归档至 dev/engine/audits/。

**与 §5.1-5.4 升级因子的关系：** §5.1-5.4 定义了五类升级因子的状态刻度与作用机制，但"何时判定升级因子已被触发"需要可观测的量化判据。本节的相关性指标族即为升级因子（尤其是市场恐慌情绪、信心崩塌类传染）的量化触发判据，并作为 [contagion-matrix.md](contagion-matrix.md) §6.2"正常环境→压力环境强度跳升量表"启用条件的量化输入。
...
**用途：** 判断行业内企业的信用利差是否同步变动——高相关性说明行业系统性风险主导，低相关性说明企业个体风险主导。

**光伏行业案例（结论指针）：** 2026-01至2026-06 光伏5只可转债行业平均ρ=0.576（中等相关——行业内部并非完全同步）；截面标准差σ_cross在6个月内从35bp扩大至62bp——行业内部分化加剧。**结论：行业"已不是一条船"**——不同企业的信用定价正在分化，行业系统性风险正在被个体风险替代，行业β在下降。完整相关性矩阵与逐对分析已随 quantitative-analysis.md 归档（dev/engine/audits/quantitative-analysis.md §5.2）。

#### 5.5.3 跨行业违约相关性突变检测（0.4 / 0.5 / 0.7 阈值）

跨行业违约相关性在正常时期通常较低，但在系统性压力时期会急剧上升——这是"风险分散失灵"的量化信号。历史锚点：2020-11永煤违约 ρ从0.3升至0.7+（恢复约6个月）；2024-05房地产冲击 ρ从0.4升至0.8+；2018-10民企违约潮 ρ从0.2升至0.6（恢复约12个月）。
...
相关性突变的统计显著性判据（|Δ| > 2×SE_Fisher 的修订规则）以 dual-track-methodology.md 附录.2.6 为单一事实源。

#### 5.5.4 区域相关性突变——永煤案例（结论指针）

永煤违约（2020-11-10）后区域利差相关性跳升是**区域共振的量化证据**：河南-山西ρ由违约前12周的0.25跳升至违约后4周的0.72（河南-河北 0.28→0.68），全国国企截面σ由22bp扩大至58bp；违约后12周回落至0.45/0.42、σ 35bp，但未回到违约前水平。事件过程与利差传导路径见本文 §2.3 与 §三 路径3；完整数据表已随 quantitative-analysis.md 归档（dev/engine/audits/quantitative-analysis.md §5.4）。

**规则：** 区域利差相关性在压力事件后跳升是系统性风险传导的量化证据。当检测到非相关区域之间的利差同步走阔时，系统性风险正在蔓延。
...
3. 检测规则：
   a) |mean_ρ 周度变化| > 2 × SE_Fisher → 预警（统计显著的相关性突变；SE_Fisher = √(1/(n₁-3) + 1/(n₂-3))，两窗口标准误合并）
   b) 任意ρ跳升 > 0.3（单周） → 检查是否存在压力事件
   c) σ_cross < 30bp 且 mean_ρ > 0.6 → 标签化风险
4. 输出：相关性状态 + 拓扑结构 + 趋势 + 建议
```

（规则 a) 以 2×SE_Fisher 统计显著性判据为准（替代旧固定 0.1 阈值），规范全文见 dual-track-methodology.md 附录.2.6。）
