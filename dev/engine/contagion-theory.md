...
归并自 quantitative-analysis.md §5.1-5.5（§5.5.1 相关性统计量置信区间规范的单一事实源为 [dual-track-methodology.md](dual-track-methodology.md) 附录.2.6，不在本节重复），v0.11.2 起本节为单一事实源。
...
**光伏行业案例（结论指针）：** 2026-01至2026-06 光伏5只可转债行业平均ρ=0.576（中等相关——行业内部并非完全同步）；截面标准差σ_cross在6个月内从35bp扩大至62bp——行业内部分化加剧。**结论：行业"已不是一条船"**——不同企业的信用定价正在分化，行业系统性风险正在被个体风险替代，行业β在下降。完整相关性矩阵与逐对分析见 quantitative-analysis.md §5.2。
...
永煤违约（2020-11-10）后区域利差相关性跳升是**区域共振的量化证据**：河南-山西ρ由违约前12周的0.25跳升至违约后4周的0.72（河南-河北 0.28→0.68），全国国企截面σ由22bp扩大至58bp；违约后12周回落至0.45/0.42、σ 35bp，但未回到违约前水平。事件过程与利差传导路径见本文 §2.3 与 §三 路径3；完整数据表见 quantitative-analysis.md §5.4。
...
（规则 a) 的统计显著性修订——以 2×SE_Fisher 替代固定 0.1 阈值——见 dual-track-methodology.md 附录.2.6。）
