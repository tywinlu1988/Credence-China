...
归并自 qualitative-analysis.md §1.3，v0.11.2 起本节为单一事实源。六类利益相关者（M0-M5）对定性分析的需求权重不同，各视角的定性分析权重和关注焦点如下：
...
M0和M5对定性分析依赖最高——这两个角色面临最大的信息不对称和长期不确定性；M3依赖最低——短期交易中量化信号占主导。
...
归并自 quantitative-analysis.md §7.2/§7.3，v0.11.2 起为单一事实源。转股溢价率存在两种计算口径，信用分析必须以**转股价值口径**为准：

| 口径 | 计算基础 | 用途定位 |
|---|---|---|
| 面值口径 | 基于面值100元的简化计算 | 交易报价指标（行业惯例），不直接反映信用风险 |
| 转股价值口径 | (转债价格 − 转股价值) / 转股价值 | 信用分析与警报判断的唯一有效口径 |

案例（隆22转债，2026-07-01）：面值口径74% vs 转股价值口径15.2%——同一时点相差近5倍，误用面值口径会把中性信号误读为信用警报。上述关键阈值（<10% 强股性 / >100% 深度价外）与一切警报判断均按转股价值口径执行。

转股价值口径的信用解读区间（归并自 quantitative-analysis.md §7.2，已归档 dev/engine/audits/）：

| 转股溢价率区间（转股价值口径） | 结合信用分析时的解读 |
|---|---|
| 0-20% | 信用风险低——市场愿意转股 |
| 20-40% | 正常信用定价 |
| 40-60% | 债底折价存在——市场对信用有隐性担忧 |
| 60-80% | 信用风险溢价正在主导定价——纯债部分已被打折扣 |
| >80% | 几乎无转股可能性——债券按违约风险定价 |

局限性：高溢价率并不总是信用问题——受正股价格、剩余期限、波动率、利率多因素影响；可转债的债底保护（回售条款）质量差异大——需要逐券分析保护条款；必须同时查看正股价格趋势，排除"仅因股价大涨导致溢价率异常"的干扰（归并自 quantitative-analysis.md §7.2 局限性标注，已归档 dev/engine/audits/）。
...
> **单源化说明**：五维集中度分析框架（阈值体系、评分规则、评级调整映射）以 [concentration-framework.md](concentration-framework.md) 为单一事实源（v0.7.0 起，`src/concentration_scorer.py` 运行时解析该文档）。本节 v0.7.0 前的初始版本内容已废止移除，避免双源漂移。
