...
| `credit-report-builder` | 把完成的信用分析装配为交付报告（选模板 Type 1–19（Type 12 已归档）、映射 L0/L1/L2 层、装配仪表盘）；需上游分析产物，自身不做分析 | `dev/.claude/skills/credit-report-builder/SKILL.md` |
...
1. **禁自造报告形式**：交付报告一律使用 `dev/templates/` 中工作路径注册表为该路径指定的模板（Type 1–19，Type 12 已归档），禁止自行设计版式或"参考模板风格自制"；无可用模板时如实告知，不自创。
