...
> 后续版本将在此表追加 report / qa 两份 skill 行（见下「四段管线」）。
...
| ③ report | 把完成的分析装配为交付报告 | `credit-report-builder` | 🔴 待开发（planned，v0.7.7 落地） |
| ④ qa | 交付前质量门复核 | `credit-qa-verifier` | 🔴 待开发（planned，v0.7.7 落地） |

report / qa 两段为 planned 阶段；本表先固化四段骨架，对应 skill 在 v0.7.7 落地。
